...
static bool writeLinesToFile(const string &path, const vector<string> &lines,
                             string &error) {
...
static bool writeRawToFile(const string &path, const string &content,
                           string &error) {
  string tmpPath = path + ".mcp_tmp";
...
json SystemTools::editor(const json &args) {
  ...
  if (command.empty()) {
    return makeTextResult("Error: 'command' is required. Available commands: "
                          "create, write, append, insert, replace, "
                          "replace_lines, delete_lines, undo, diff, read",
                          true);
  }
  if (path.empty()) {
  ...
  try {
    ...
    if (command == "create") {
      ...
      if (fs::exists(path) && !overwrite) {
        return makeTextResult("Error: File already exists: " + path +
    ...
    // ==================== INSERT ====================
      ...
      // Insert the lines
      lines.insert(lines.begin() + insertPos, insertLines.begin(),
                   insertLines.end());
      ...
      ostringstream oss;
      oss << "✅ Inserted " << insertLines.size() << " line(s) at line "
          << lineNum << " in: " << path << "\n";
    ...
    // ==================== REPLACE_LINES ====================
      ...
      if (startLine < 1 || endLine < 1) {
        return makeTextResult(
            "Error: 'start_line' and 'end_line' must be >= 1 (1-indexed)",
            true);
      }
      if (startLine > endLine) {
        return makeTextResult("Error: 'start_line' must be <= 'end_line'",
                              true);
      }
      if (!fs::exists(path)) {
        return makeTextResult("Error: File does not exist: " + path, true);
      ...
      if (startLine > (int)lines.size()) {
        return makeTextResult("Error: start_line (" + to_string(startLine) +
                                  ") exceeds file length (" +
      ...
      lines.insert(lines.begin() + (startLine - 1), newLines.begin(),
                   newLines.end());
      ...
      ostringstream oss;
      oss << "✅ Replaced lines " << startLine << "-" << actualEnd << " ("
          << removedCount << " line(s)) with " << newLines.size()
          << " new line(s)\n";
      oss << "File: " << path << "\n";
      oss << "Total lines: " << lines.size();
      return makeTextResult(oss.str());
    ...
    // ==================== DELETE_LINES ====================
      ...
      ostringstream oss;
      oss << "✅ Deleted lines " << startLine << "-" << actualEnd << " ("
          << removedCount << " line(s))\n";
      oss << "File: " << path << "\n";
    ...
    // ==================== UNDO ====================
      ...
      if (!fs::exists(bakPath)) {
        return makeTextResult(
            "Error: No backup found for: " + path + " (nothing to undo)", true);
      }
      ...
      if (!createBackup(path, backupErr)) {
        ...
        cerr << "[Editor] Warning: couldn't backup current before undo: "
             << backupErr << endl;
      }
    ...
    // ==================== DIFF ====================
      ...
      if (!fs::exists(bakPath)) {
        return makeTextResult(
            "No backup found for: " + path + " (no changes tracked)", false);
      }
      if (!fs::exists(path)) {
        return makeTextResult("Error: File does not exist: " + path, true);
      }
      ...
      ostringstream oss;
      oss << "Diff for: " << path << "\n";
      oss << "Backup: " << bakLines.size()
          << " lines | Current: " << curLines.size() << " lines\n";
      oss << string(60, '=') << "\n";
    ...
    // ==================== UNKNOWN COMMAND ====================
      ...
      return makeTextResult(
          "Error: Unknown command '" + command +
              "'. Available commands: create, write, append, insert, replace, "
              "replace_lines, delete_lines, undo, diff, read",
          true);
    }
    ...
    return makeTextResult(
        "Error in editor (" + command + "): " + string(e.what()), true);
  }
}
