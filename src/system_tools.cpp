...
json SystemTools::getToolDefinitions() const {
  return json::array(
      {// 1. list_directory
       {{"name", "list_directory"},
        {"description",
         "List all files and directories at a given path. Returns name, type "
         "(file/directory), and size for each entry."},
        {"inputSchema",
         {{"type", "object"},
          {"properties",
           {{"path",
             {{"type", "string"},
              {"description",
               "Absolute path to the directory to list. Defaults to home "
               "directory if not specified."}}}}},
          {"required", json::array({"path"})}}}},
       // 2. read_file
       {{"name", "read_file"},
        {"description", "Read the contents of a text file. Can read the entire "
                        "file or a specific line range."},
        {"inputSchema",
         {{"type", "object"},
          {"properties",
           {{"path",
            ...
            {"start_line",
             {{"type", "integer"},
              {"description", "Optional start line (1-indexed). If omitted, "
                              "reads from the beginning."}}},
            {"end_line",
             {{"type", "integer"},
              {"description", "Optional end line (1-indexed, inclusive). If "
                              "omitted, reads to the end."}}}}},
          {"required", json::array({"path"})}}}},
       // 3. search_files
       {{"name", "search_files"},
         ...
         {{"type", "object"},
          {"properties",
           {{"path",
            ...
            {"pattern",
             {{"type", "string"},
              {"description", "Search pattern (substring match on filename). "
                              "For example: '.cpp', 'main', 'config'"}}},
            {"max_results",
             {{"type", "integer"},
       ...
       {{"name", "get_file_info"},
        {"description", "Get detailed metadata about a file or directory: "
                        "size, permissions, owner, modified time, type."},
        {"inputSchema",
         {{"type", "object"},
          {"properties",
           {{"path",
       ...
       {{"name", "get_system_info"},
        {"description", "Get system information: OS, kernel, hostname, CPU, "
                        "memory usage, disk usage."},
        {"inputSchema", {{"type", "object"}, {"properties", json::object()}}}},
       // 6. list_processes
       {{"name", "list_processes"},
        {"description",
         "List running processes with PID, name, CPU usage, and memory usage."},
        {"inputSchema",
         {{"type", "object"},
          {"properties",
           {{"sort_by",
             {{"type", "string"},
              {"description",
               "Sort by: 'cpu', 'mem', or 'pid'. Default: 'cpu'"},
       ...
       {{"name", "list_installed_apps"},
        {"description",
         "List installed desktop applications on this Linux system."},
        {"inputSchema",
         {{"type", "object"},
          {"properties",
           {{"search",
             {{"type", "string"},
              {"description",
               "Optional search filter for application name"}}}}}}}},
       // 8. run_command
       {{"name", "run_command"},
        {"description", "Execute a shell command and return its output (stdout "
                        "and stderr). Use with caution."},
        {"inputSchema",
         {{"type", "object"},
          {"properties",
           {{"command",
       ...
       {{"name", "get_history"},
        {"description",
         "Get the list of recently executed tool calls (summary)"},
        {"inputSchema",
         {{"type", "object"},
          {"properties",
           {{"limit",
       ...
       {{"name", "get_history_by_id"},
        {"description",
         "Get detailed history including full result by session/title ID"},
        {"inputSchema",
         {{"type", "object"},
          {"properties",
           {{"id",
       ...
       {{"name", "editor"},
        {"description",
         "Powerful file editor with multiple operations. Commands: "
         "'create' - Create new file (args: path, content, overwrite). "
         "'write' - Write entire file content, always overwrites (args: path, "
         "content). "
         "'append' - Append content to end of file (args: path, content). "
         "'insert' - Insert content at specific line number (args: path, line, "
         "content). "
         "'replace' - Search and replace text in file (args: path, search, "
         "replacement, all). "
         "'replace_lines' - Replace a range of lines (args: path, start_line, "
         "end_line, content). "
         "'delete_lines' - Delete a range of lines (args: path, start_line, "
         "end_line). "
         "'undo' - Undo last change from backup (args: path). "
         "'diff' - Show differences between backup and current file (args: "
         "path). "
         "'read' - Read file content (args: path, start_line, end_line). "
         "All write operations use atomic writes (temp file + rename) to "
         "prevent corruption. "
         "A backup (.mcp_bak) is created before every modification for undo "
         "support."},
        {"inputSchema",
         {{"type", "object"},
          {"properties",
           {{"command",
             {{"type", "string"},
              ...
              {"enum", json::array({"create", "write", "append", "insert",
                                    "replace", "replace_lines", "delete_lines",
                                    "undo", "diff", "read"})}}},
            {"path",
             {{"type", "string"},
            ...
            {"content",
             {{"type", "string"},
              {"description",
               "File content for create/write/append/insert/replace_lines"}}},
            {"line",
             {{"type", "integer"},
            ...
            {"start_line",
             {{"type", "integer"},
              {"description",
               "Start line for replace_lines/delete_lines/read (1-indexed)"}}},
            {"end_line",
             {{"type", "integer"},
              {"description", "End line for replace_lines/delete_lines/read "
                              "(1-indexed, inclusive)"}}},
            {"search",
             {{"type", "string"},
            ...
            {"all",
             {{"type", "boolean"},
              {"description",
               "Replace all occurrences (replace command, default: false)"}}},
            {"overwrite",
             {{"type", "boolean"},
              {"description",
               "Overwrite existing file (create command, default: false)"}}}}},
          {"required", json::array({"command", "path"})}}}},
       // 12 Office Wizard
       {{"name", "office_wizard"},
        {"description",
         "Create an office document (Word, Excel, PowerPoint) with advanced "
         "custom configurations and layouts using native C++ and libxml2."},
        {"inputSchema",
         {{"type", "object"},
          {"properties",
           {{"document_type",
             {{"type", "string"},
              ...
              {"enum", json::array({"word", "excel", "powerpoint"})}}},
            {"file_path",
             {{"type", "string"},
            ...
            {"config",
             {{"type", "object"},
              {"description",
               "Configuration data for formatting and content of the document"},
              {"properties",
               {{"margins",
                 {{"type", "object"},
                  {"description", "Margins in inches for Word document"},
                  {"properties",
                   {{"top", {{"type", "number"}}},
                    {"bottom", {{"type", "number"}}},
                    {"left", {{"type", "number"}}},
                ...
                {"paragraphs",
                 {{"type", "array"},
                  {"description", "List of paragraphs for Word document"},
                  {"items",
                   {{"type", "object"},
                    {"properties",
                     {{"text",
                       {{"type", "string"},
                        {"description", "Text content of paragraph. If "
                                        "combined with image_url, this acts as "
                                        "the caption below the image."}}},
                      {"align",
                       {{"type", "string"},
                        {"enum",
                         json::array({"left", "center", "right", "justify"})}}},
                      {"bold", {{"type", "boolean"}}},
                      {"italic", {{"type", "boolean"}}},
                      {"font_size",
                       {{"type", "integer"},
                      ...
                      {"spacing_after",
                       {{"type", "integer"},
                        {"description",
                         "Spacing after paragraph in twips (1/20 of a pt)"}}},
                      {"heading",
                       {{"type", "string"},
                        {"enum", json::array({"h1", "h2", "h3", "normal"})}}},
                      {"image_url",
                       {{"type", "string"},
                        {"description", "Optional URL to embed a direct image. "
                                        "The image will be downloaded and "
                                        "bound internally to the document."}}},
                      {"image_width",
                       {{"type", "integer"},
                        {"description",
                         "Display width of image in pixels (default: 300)"}}},
                      {"image_height",
                       {{"type", "integer"},
                        {"description", "Display height of image in pixels "
                                        "(default: 200)"}}}}}}}}},
                {"sheets",
                 {{"type", "array"},
                  {"description", "List of sheets for Excel document"},
                  {"items",
                   {{"type", "object"},
                    {"properties",
                     {{"name", {{"type", "string"}}},
                           ...
                           {{"type", "object"},
                            {"properties",
                             {{"value", {{"type", "string"}}},
                              {"type",
                               {{"type", "string"},
                                {"enum", json::array({"string", "number"})}}},
                              {"bold", {{"type", "boolean"}}}}}}}}}}}}}}}}},
                {"slides",
                 {{"type", "array"},
                  {"description", "List of slides for PowerPoint document"},
                  {"items",
                   {{"type", "object"},
                    {"properties",
                     {{"title", {{"type", "string"}}},
                      {"bullets",
                       {{"type", "array"},
                        {"items", {{"type", "string"}}}}}}}}}}}}}}}}},
          {"required", json::array({"document_type", "file_path"})}}}},
        // 13. FTP Handler
        {{"name", "ftp_handler"},
         {"description",
          "Manage files on a remote FTP server. Commands: "
          "'list' - List files/directories at a remote path (args: "
          "remote_path). "
          "'read' - Download and read a remote file (args: remote_path). "
          "'upload' - Upload content to a remote file (args: remote_path, "
          "content). "
          "'edit' - Download, search-and-replace, re-upload a remote file "
          "(args: remote_path, search, replacement). "
          "'delete' - Delete a remote file (args: remote_path). "
          "'delete_dir' - Remove a remote directory (args: remote_path). "
          "'rename' - Rename/move a remote file (args: remote_path, new_path). "
          "'mkdir' - Create a remote directory (args: remote_path). "
          "'info' - Get file size and modification time (args: remote_path). "
          "All downloaded files are cached locally under ~/.mcp/ftp/ for "
          "preview. Set encoding='base64' on upload/edit/read to transfer "
          "code safely without quote/indentation corruption."},
         {"inputSchema",
          {{"type", "object"},
           {"properties",
            {{"command",
              {{"type", "string"},
               {"description", "The FTP command to execute"},
               {"enum",
                json::array({"list", "read", "upload", "edit", "delete",
                             "delete_dir", "rename", "mkdir", "info"})}}},
             {"remote_path",
              {{"type", "string"},
               {"description", "Remote path on the FTP server (e.g. "
                               "/public_html/index.html)"}}},
             {"content",
              {{"type", "string"},
               {"description", "File content for upload command"}}},
             {"search",
              {{"type", "string"},
               {"description", "Text to search for (edit command)"}}},
             {"replacement",
              {{"type", "string"},
               {"description", "Replacement text (edit command)"}}},
             {"new_path",
              {{"type", "string"},
               {"description", "New remote path for rename command"}}},
             {"encoding",
              {{"type", "string"},
               {"description",
                "Set to 'base64' to Base64-decode content (upload) and "
                "search/replacement (edit) on write, and to return Base64 on "
                "read. Use for code containing quotes, $, backticks or "
                "indentation to avoid escaping corruption. Default: plain."},
               {"enum", json::array({"plain", "base64"})}}}}},
           {"required", json::array({"command"})}}}}});
}
...
json SystemTools::callTool(const string &name, const json &arguments) {
  ...
  if (name == "get_system_info")
    return getSystemInfo(arguments);
  if (name == "list_processes")
    return listProcesses(arguments);
  if (name == "list_installed_apps")
    return listInstalledApps(arguments);
  if (name == "run_command")
    return runCommand(arguments);
  if (name == "editor")
    return editor(arguments);
  if (name == "get_history")
    return getHistory(arguments);
  if (name == "get_history_by_id")
    return getHistoryById(arguments);
  if (name == "office_wizard")
    return officeWizard(arguments);
  if (name == "ftp_handler")
    return ftpHandler(arguments);
...
json SystemTools::listDirectory(const json &args) {
  ...
  try {
    ...
    ostringstream oss;
    oss << "Directory: " << path << "\n\n";
    oss << left << setw(50) << "Name" << setw(12) << "Type"
        << "Size" << "\n";
    oss << string(75, '-') << "\n";
    ...
    for (const auto &entry : entries) {
      ...
      if (entry.is_regular_file()) {
        ...
        if (sz < 1024)
          ...
          size = to_string(sz / 1024) + " KB";
        else if (sz < 1024 * 1024 * 1024)
          size = to_string(sz / (1024 * 1024)) + " MB";
        else
          size = to_string(sz / (1024 * 1024 * 1024)) + " GB";
      }

      oss << left << setw(50) << name << setw(12) << type << size << "\n";
    }
  ...
  try {
    ...
    if (fileSize > 1024 * 1024) {
      return makeTextResult(
          "Error: File too large (" + to_string(fileSize) +
              " bytes). Max 1MB. Use start_line/end_line to read portions.",
          true);
    }
...
json SystemTools::searchFiles(const json &args) {
  ...
  try {
    ...
    for (auto it = fs::recursive_directory_iterator(
             path, fs::directory_options::skip_permission_denied);
         it != fs::recursive_directory_iterator() && count < maxResults; ++it) {
      try {
...
json SystemTools::getFileInfo(const json &args) {
  ...
  try {
    ...
    auto sctime =
        chrono::system_clock::to_time_t(chrono::file_clock::to_sys(ftime));
...
json SystemTools::getSystemInfo(const json &args) {
  ...
  oss << "CPU:          " << exec("wmic cpu get name /value | findstr Name")
      << "\n";
  oss << "Memory (Total):"
      << exec("systeminfo | findstr /C:\"Total Physical Memory\"") << "\n";
#else
  // Hostname
  ...
  // OS
  string os = exec("cat /etc/os-release 2>/dev/null | grep PRETTY_NAME | cut "
                   "-d= -f2 | tr -d '\"'");
  if (!os.empty() && os.back() == '\n')
    os.pop_back();
  oss << "OS:           " << os << "\n";
  ...
  // CPU
  string cpu = exec("cat /proc/cpuinfo 2>/dev/null | grep 'model name' | head "
                    "-1 | cut -d: -f2");
  if (!cpu.empty() && cpu[0] == ' ')
    cpu = cpu.substr(1);
  ...
  // Memory
  string mem = exec("free -h 2>/dev/null | grep Mem | awk '{print \"Total: \" "
                    "$2 \", Used: \" $3 \", Free: \" $4}'");
  ...
  // Disk
  string disk = exec("df -h / 2>/dev/null | tail -1 | awk '{print \"Total: \" "
                     "$2 \", Used: \" $3 \" (\" $5 \"), Free: \" $4}'");
...
json SystemTools::listProcesses(const json &args) {
  ...
  string cmd =
      "ps aux " + sortFlag + " 2>/dev/null | head -" + to_string(limit + 1);
  string output = exec(cmd);
  ...
  return makeTextResult("Running Processes (sorted by " + sortBy + ", top " +
                        to_string(limit) + "):\n\n" + output);
#endif
}
...
#ifdef _WIN32
  return makeTextResult("list_installed_apps is not yet supported on Windows",
                        true);
#else
      ...
      if (entry.path().extension() == ".desktop") {
        ...
        if (!appName.empty()) {
          ...
          if (!search.empty()) {
            ...
            transform(lowerName.begin(), lowerName.end(), lowerName.begin(),
                      ::tolower);
            transform(lowerSearch.begin(), lowerSearch.end(),
        ...
        for (const auto &entry : fs::directory_iterator(userAppsDir)) {
          if (entry.path().extension() == ".desktop") {
            ...
            while (getline(file, line)) {
              ...
              if (inDesktopEntry && line.substr(0, 5) == "Name=" &&
                  appName.empty()) {
                appName = line.substr(5);
              }
            }
...
#else
  string cmd = "timeout " + to_string(timeout) + " bash -c " + "'" + command +
               "'" + " 2>&1";
#endif
...
json SystemTools::getHistory(const json &args) {
  ...
  sqlite3_stmt *stmt;
  string sql = "SELECT id, mainContext, timestamp FROM history_title ORDER BY "
               "id DESC LIMIT ?";
