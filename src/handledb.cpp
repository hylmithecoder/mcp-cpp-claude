...
void DataBase::initDb(const char *dbPath) {
  ...
  rc = sqlite3_exec(db,
                    "CREATE TABLE IF NOT EXISTS history_title (id INTEGER "
                    "PRIMARY KEY AUTOINCREMENT, mainContext TEXT NOT NULL, "
  ...
  rc = sqlite3_exec(
      db,
      "CREATE TABLE IF NOT EXISTS history_context (id INTEGER PRIMARY KEY "
      "AUTOINCREMENT, id_title INTEGER ,context TEXT NOT NULL, response TEXT "
...
int DataBase::insertData(FormatInput data, sqlite3 *db) {
  sqlite3_stmt *stmt;
  const char *sql1 =
      "INSERT INTO history_title (mainContext, timestamp) VALUES (?, ?)";
  ...
  const char *sql2 = "INSERT INTO history_context (id_title, context, "
...
std::string DataBase::readTheContext(sqlite3 *db, int idTitle) {
  ...
  std::string sql =
      "SELECT mainContext, timestamp FROM history_title WHERE id = ?";
  ...
  sql = "SELECT context, response, timestamp FROM history_context WHERE "
        "id_title = ?";
  rc = sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr);
  if (rc == SQLITE_OK) {
    sqlite3_bind_int(stmt, 1, idTitle);
